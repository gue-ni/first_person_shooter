...
export class Factory {
	...
	createPrimaryWeapon(player, network) {
		...
		primary.addComponent(
			new MuzzleFlash(primary, new THREE.Vector3(0.1, -0.2, -0.7), this.listener, new Smoke(this.scene))
		);
		primary.addComponent(
			new SimpleGunModel(primary, "./assets/objects/AUG2.glb", {
	...
	createSecondaryWeapon(player, network) {
		...
		secondary.addComponent(
			new MuzzleFlash(secondary, new THREE.Vector3(0.1, -0.4, -1.2), this.listener, new Smoke(this.scene))
		);
